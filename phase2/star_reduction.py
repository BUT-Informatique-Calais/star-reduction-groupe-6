...
# Normalize to [0, 1] using float64 for maximum precision
data_normalized = (data_gray - data_gray.min()) / (data_gray.max() - data_gray.min())
image_original = data_normalized.astype(np.float64)

# Save original
plt.imsave('./results/image_original.png', image_original, cmap='gray')
...
mask = np.zeros_like(data_gray, dtype=np.float64)
...
# Apply erosion to the entire image (reduced: 3x3, 1 iteration)
kernel = np.ones((3, 3), np.float64)
image_eroded = cv.erode(image_original, kernel, iterations=1)

# Save eroded image
plt.imsave('./results/image_erodee.png', image_eroded, cmap='gray')

# Combine using mask: I_final = M × I_erode + (1-M) × I_original
# Work directly in float64 for maximum precision
image_final = (mask_blurred * image_eroded) + ((1 - mask_blurred) * image_original)
